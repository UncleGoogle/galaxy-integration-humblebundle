...
class WindowsRegistryClient:
    ...
    def __init__(self):
        ...
        self.__uk_hive_len: Dict[str, int] = {}
        ...
        for arch_key in self._ARCH_KEYS:
            for hive in self._LOOKUP_REGISTRY_HIVES:
                with winreg.OpenKey(hive, self._UNINSTALL_LOCATION, 0, winreg.KEY_READ | arch_key) as key:
                    ...
                    if subkeys == self.__uk_hive_len.get(hive | arch_key):
                        continue  # probably no changes since last check
                    self.__uk_hive_len[hive | arch_key] = subkeys
                ...
                for og in owned_games:
                    if self.__matches(og.human_name, uk):
                        ...
                        if exe is not None:
                            ...
                            # owned_games.remove(og)  # premature opt.
                            break
