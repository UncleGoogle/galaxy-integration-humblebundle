...
async def test_trove(api_mock, plugin):
    api_mock.get_subscriber_hub_data.return_value = {
        "subscriptionJoinDate|datetime": "2019-07-18T13:58:14.730512",
        "subscriptionExpires|datetime": "2021-10-05T17:00:00",
    }
    time_2020_10_29 = 1603991800
    time_2021_10_12 = 1633991826
    parsed_from_page = {
        'newlyAdded': [
            {'human-name': 'A', 'machine_name': 'a'},
            {'human-name': 'Z', 'machine_name': 'z', 'date-added': time_2021_10_12},
        ]
    }
    ...
    chunks_from_api = [
        [
            {'human-name': 'A', 'machine_name': 'a'},
            {'human-name': 'B', 'machine_name': 'b', 'date-added': time_2020_10_29},
        ],
        [
            {'human-name': 'C', 'machine_name': 'c', 'date-added': None},
    ...
    assert sorted(trove_games, key=lambda x: x.game_id) \
        == sorted([
            ...
            SubscriptionGame('B', 'b', start_time=time_2020_10_29),
    ...
    assert 'Z' not in [g.game_title for g in trove_games], 'Returned Trove game added after subscription expired'
